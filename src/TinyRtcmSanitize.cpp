...
Status sanitizeRewriteLocationFrame(uint8_t* frame, size_t len, size_t cap, size_t* outLen) {
  ...
  if (t == 1005) return rewrite1005ToPublishIdentity(frame, len, frame, cap, outLen);
  if (t == 1006) return rewrite1006ToPublishIdentity(frame, len, frame, cap, outLen);
  if (t == 1033) return rewrite1033ToSanitized(frame, len, frame, cap, outLen);
  *outLen = len;
  return Status::Ok;
}
